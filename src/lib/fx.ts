...
export interface CamFx {
  ...
  dither: { on: boolean; algo: number; scale: number; threshold: number; audio: number };
  edge: { on: boolean; strength: number; glow: number; invert: boolean; audio: number };
  ...
  pixelsort: { on: boolean; amount: number; threshold: number };
  chroma: { on: boolean; amount: number; audio: number };
  scan: { on: boolean; intensity: number; count: number; crt: boolean };
...
export function defaultCamFx(): CamFx {
  return {
    ...
    dither: { on: false, algo: 0, scale: 1, threshold: 0.5, audio: 0.4 },
    edge: { on: false, strength: 4, glow: 1, invert: false, audio: 0.5 },
    ...
    pixelsort: { on: false, amount: 0.4, threshold: 0.6 },
    chroma: { on: true, amount: 0.3, audio: 0.7 },
    scan: { on: false, intensity: 0.4, count: 700, crt: false },
    bloom: { on: true, amount: 0.5, audio: 0.8 },
